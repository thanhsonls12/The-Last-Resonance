...
var decor_nodes := []
var archive_power_lights: Array[Dictionary] = []
var archive_power_materials: Array[Dictionary] = []
var _is_archive_powered: bool = false
...
func build(logic: GameLogic, decorations := []) -> void:
	...
	energy_nodes.clear()
	decor_nodes.clear()
	archive_power_lights.clear()
	archive_power_materials.clear()
	...
	var floor_mat := _mat(COLOR_FLOOR)
	var platform_mat := _mat(COLOR_PLATFORM)
	var platform_edge_mat := _mat(COLOR_PLATFORM_EDGE)
	var grid_mat := _mat(COLOR_GRID)
	var wall_mat := _mat(COLOR_WALL)
	var wall_edge_mat := _mat(COLOR_WALL_EDGE)
	var slot_mat := _mat(Color(0.20, 0.06, 0.025), 0.15)
	var slot_ring_mat := _mat(COLOR_GOAL, 2.4)
	var plate_mat := _mat(Color(0.18, 0.12, 0.025), 0.15)
	var plate_ring_mat := _mat(COLOR_PLATE, 2.2)
	var door_mat := _mat(Color(0.20, 0.035, 0.035))
	var door_edge_mat := _mat(COLOR_DOOR, 2.0)
	lock_cable_material = _mat(Color(0.55, 0.04, 0.04), 0.65)
	var portal_mat := _mat(Color(0.08, 0.025, 0.16), 0.25)
	var portal_ring_mat := _mat(COLOR_PORTAL, 3.0)
	var elevator_mat := _mat(Color(0.025, 0.16, 0.13), 0.25)
	var elevator_ring_mat := _mat(COLOR_ELEVATOR, 2.8)
	var bridge_mat := _mat(Color(0.18, 0.05, 0.025), 0.2)
	var bridge_ring_mat := _mat(COLOR_BRIDGE, 2.6)
	var energy_mat := _mat(Color(0.025, 0.16, 0.14), 0.25)
	var energy_ring_mat := _mat(COLOR_ENERGY, 3.0)
	var block_mat := _mat(COLOR_BLOCK)
	var block_edge_mat := _mat(COLOR_BLOCK_EDGE, 2.0)
	var player_mat := _mat(COLOR_PLAYER, 1.2)
	var player_edge_mat := _mat(Color(0.65, 0.95, 1.0), 2.8)

	_build_forgotten_archive(logic)
	var board_bounds := _board_bounds(logic)

	for v in logic.floors.keys():
		if not logic.walls.has(v):
			_box(self, world_position(v) + Vector3(0, -0.18, 0), Vector3(0.98, 0.34, 0.98), platform_mat)
			if _platform_edge(logic, v, Vector3i(1, 0, 0)):
				_box(self, world_position(v) + Vector3(0.49, -0.18, 0), Vector3(0.035, 0.34, 0.92), platform_edge_mat)
			if _platform_edge(logic, v, Vector3i(-1, 0, 0)):
				_box(self, world_position(v) + Vector3(-0.49, -0.18, 0), Vector3(0.035, 0.34, 0.92), platform_edge_mat)
			if _platform_edge(logic, v, Vector3i(0, 0, 1)):
				_box(self, world_position(v) + Vector3(0, -0.18, 0.49), Vector3(0.92, 0.34, 0.035), platform_edge_mat)
			if _platform_edge(logic, v, Vector3i(0, 0, -1)):
				_box(self, world_position(v) + Vector3(0, -0.18, -0.49), Vector3(0.92, 0.34, 0.035), platform_edge_mat)
		if not logic.walls.has(v):
			var tile := _spawn(self, FLOOR_TILE_PATH, world_position(v), world_position(v).y)
			if tile == null:
				_box(self, world_position(v), Vector3(0.98, 0.08, 0.98), floor_mat)
				_box(self, world_position(v) + Vector3(0, 0.065, -0.43), Vector3(0.72, 0.018, 0.018), grid_mat)
				_box(self, world_position(v) + Vector3(-0.43, 0.065, 0), Vector3(0.018, 0.018, 0.72), grid_mat)
	for v in logic.walls.keys():
		...
		if on_x_edge and on_z_edge:
			...
			if corner == null:
				_box(self, world_position(v) + Vector3(0, 0.46, 0), Vector3(1, 1, 1), wall_mat)
			continue
		if on_x_edge or on_z_edge:
			...
			if panel != null:
				...
				continue
			_box(self, world_position(v) + Vector3(0, 0.31, 0), Vector3(0.94, 0.62, 0.94), wall_mat)
			continue
		var pillar := _spawn(self, WALL_PILLAR_PATH, world_position(v), FLOOR_TOP_Y)
		if pillar == null:
			_box(self, world_position(v) + Vector3(0, 0.46, 0), Vector3(1, 1, 1), wall_mat)
			_box(self, world_position(v) + Vector3(0, 0.965, 0), Vector3(0.82, 0.035, 0.82), wall_edge_mat)
	for v in logic.slots.keys():
		...
		if pedestal == null:
			_cylinder(slot_root, Vector3.ZERO, 0.30, 0.025, slot_mat)
			_torus(slot_root, Vector3(0, 0.025, 0), 0.24, 0.30, slot_ring_mat)
	...
	for v in logic.plates.keys():
		...
		if plate == null:
			_cylinder(plate_root, Vector3.ZERO, 0.34, 0.035, plate_mat)
		var status_mat := plate_ring_mat.duplicate() as StandardMaterial3D
		_torus(plate_root, Vector3(0, 0.045, 0), 0.25, 0.34, status_mat)
	...
	for v in logic.portals.keys():
		...
		_cylinder(portal_root, Vector3.ZERO, 0.31, 0.025, portal_mat)
		_torus(portal_root, Vector3(0, 0.035, 0), 0.23, 0.31, portal_ring_mat)
	...
	for v in logic.elevators.keys():
		...
		_cylinder(elevator_root, Vector3.ZERO, 0.34, 0.035, elevator_mat)
		_torus(elevator_root, Vector3(0, 0.03, 0), 0.25, 0.34, elevator_ring_mat)
	...
	for v in logic.bridges.keys():
		...
		_box(bridge_root, Vector3.ZERO, Vector3(0.84, 0.08, 0.84), bridge_mat)
		_box(bridge_root, Vector3(-0.38, 0.18, 0), Vector3(0.045, 0.28, 0.84), bridge_ring_mat)
		_box(bridge_root, Vector3(0.38, 0.18, 0), Vector3(0.045, 0.28, 0.84), bridge_ring_mat)
	...
	for v in logic.energy_nodes:
		...
		_cylinder(energy_root, Vector3.ZERO, 0.28, 0.025, energy_mat)
		_torus(energy_root, Vector3(0, 0.035, 0), 0.21, 0.30, energy_ring_mat)
	...
	set_archive_powered(false, true)
...
func _apply_power_baseline() -> void:
	...
	if is_equal_approx(gain, 1.0):
		return
	for info in archive_power_lights:
		info["off"] = minf(float(info["off"]) * gain, float(info["on"]))
	for info in archive_power_materials:
		info["off"] = minf(float(info["off"]) * gain, float(info["on"]))
...
func _build_lock_trace(from: Vector3i, to: Vector3i, material: StandardMaterial3D) -> void:
	...
	if not is_equal_approx(corner.z, finish.z):
		_box(
			...
			material)
	_sphere(self, world_position(to) + Vector3(0, 0.16, 0), 0.055, material)
...
func _build_decorations(decorations: Array) -> void:
	for deco in decorations:
		...
		if kind == "lamp":
			_add_archive_lamp(center + Vector3(0, 0.78, 0), COLOR_ARCHIVE_AMBER)
		elif kind == "holo" or kind == "terminal":
			_add_hologram_glow(center + Vector3(0, 0.48, 0))


func _build_forgotten_archive(logic: GameLogic) -> void:
	...
	var steel := _mat(COLOR_ARCHIVE_STEEL)
	steel.metallic = 0.72
	steel.roughness = 0.32
	var panel := _mat(COLOR_ARCHIVE_PANEL)
	panel.metallic = 0.55
	panel.roughness = 0.42
	var recess := _mat(Color(0.012, 0.018, 0.035))
	recess.metallic = 0.25
	recess.roughness = 0.7

	# A deep, layered plinth makes the grid read as a real room suspended inside
	# the dead Archive instead of a collection of floating tiles.
	_box(self, center + Vector3(0, -0.49, 0), Vector3(width + 0.9, 0.62, depth + 0.9), steel)
	_box(self, center + Vector3(0, -0.83, 0), Vector3(width + 0.3, 0.16, depth + 0.3), recess)
	for x in range(int(min_x), int(max_x) + 1, 2):
		_box(self, Vector3(float(x), -0.54, max_z + 0.58), Vector3(1.42, 0.25, 0.11), panel)
	for z in range(int(min_z), int(max_z) + 1, 2):
		_box(self, Vector3(max_x + 0.58, -0.54, float(z)), Vector3(0.11, 0.25, 1.42), panel)

	# The north/west archive facades sit behind the default isometric view. Their
	# uneven silhouette sells centuries of collapse without hiding walkable cells.
	_build_archive_facade(Vector3(center.x, 1.18, min_z - 0.78), Vector3(width + 1.5, 2.55, 0.34), false, panel, recess)
	_build_archive_facade(Vector3(min_x - 0.78, 1.08, center.z), Vector3(0.34, 2.35, depth + 1.5), true, panel, recess)
	...
	for corner in [
		...
		_box(self, corner, Vector3(0.48, corner.y * 2.0, 0.48), steel)
	...
	# of light from above.
	_box(self, Vector3(center.x - 1.2, 3.05, min_z - 0.68), Vector3(width - 1.4, 0.18, 0.24), steel)
	_box(self, Vector3(min_x - 0.68, 2.92, center.z - 0.8), Vector3(0.24, 0.18, depth - 1.2), steel)
	_box(self, Vector3(max_x - 0.8, 3.18, min_z - 0.68), Vector3(1.4, 0.16, 0.22), panel)
...
func _build_distant_structures(center: Vector3, width: float, depth: float, _panel: Material, _steel: Material, _recess: Material) -> void:
	var tower_mat := _mat(Color(0.09, 0.15, 0.24))
	tower_mat.metallic = 0.65
	tower_mat.roughness = 0.35
	var tower_recess := _mat(Color(0.035, 0.065, 0.11))
	var tower_panel := _mat(Color(0.06, 0.10, 0.17))
	...
	for offset in [
		...
		_box(self, center + offset, Vector3(1.6, 9.0, 1.6), tower_mat)
		_box(self, center + offset + Vector3(0, -1.0, 0), Vector3(1.8, 4.0, 1.8), tower_recess)
	...
	for cable_x in [-width * 0.35, 0.0, width * 0.35]:
		_box(self, center + Vector3(cable_x, -4.2, depth * 0.52), Vector3(0.08, 6.5, 0.08), tower_recess)
		_box(self, center + Vector3(cable_x, -4.2, -depth * 0.52), Vector3(0.08, 6.5, 0.08), tower_recess)
	...
	for t in towers:
		...
		_box(self, t_pos, t_size, tower_mat)
		_box(self, t_pos + Vector3(0, 0, 0.05), Vector3(t_size.x * 0.82, t_size.y * 0.94, t_size.z * 1.02), tower_recess)
		_box(self, t_pos + Vector3(0, t_size.y * 0.22, 0.1), Vector3(t_size.x * 0.68, t_size.y * 0.38, t_size.z * 1.04), tower_panel)
		...
		var beacon := _sphere(self, beacon_pos, 0.18, _mat(strip_color, 3.2))
		if beacon:
...
func _add_cosmic_starfield(center: Vector3) -> void:
	...
	var star_mat := _transparent_mat(Color(1.0, 1.0, 1.0, 0.95), 3.2)
	...
	for i in star_coords.size():
		...
		var star_sphere := _sphere(self, pos, 0.035 + (i % 3) * 0.015, _mat(col, 4.2))
...
func _add_static_neon_strip(position: Vector3, size: Vector3, color: Color, energy := 2.2) -> void:
	var mat := _mat(color, energy)
	_box(self, position, size, mat)





func _build_archive_facade(position: Vector3, size: Vector3, along_z: bool, panel: Material, recess: Material) -> void:
	_box(self, position, size, recess)
	var count := 5
	for i in count:
		...
		_box(self, pos, module_size, panel)
		if i % 2 == 0:
...
func _add_dust_volume(position: Vector3, extents: Vector3) -> void:
	...
	var dust_mat := _transparent_mat(Color(0.42, 0.70, 0.82, 0.23), 0.5)
...
func _add_light_shaft(position: Vector3, radius: float) -> void:
	...
	shaft.material_override = _transparent_mat(Color(0.16, 0.62, 0.90, 0.055), 0.7)
	add_child(shaft)
...
func _add_archive_lamp(position: Vector3, color: Color) -> void:
	...
	add_child(light)
	archive_power_lights.append({"node": light, "off": 0.22, "on": 2.1})
	var lens_mat := _mat(color, 0.25)
	var lens := _sphere(self, position, 0.055, lens_mat)
	lens.scale = Vector3(1.0, 0.55, 1.0)
	archive_power_materials.append({"material": lens_mat, "off": 0.25, "on": 4.0})
...
func _add_hologram_glow(position: Vector3) -> void:
	...
	add_child(light)
	archive_power_lights.append({"node": light, "off": 0.28, "on": 2.7})
	var holo_mat := _transparent_mat(Color(0.10, 0.86, 1.0, 0.16), 1.2)
	_cylinder(self, position + Vector3(0, 0.46, 0), 0.22, 0.82, holo_mat)
	for height in [0.12, 0.40, 0.68]:
		_torus(self, position + Vector3(0, height, 0), 0.20, 0.225, holo_mat)
	archive_power_materials.append({"material": holo_mat, "off": 0.35, "on": 3.6})


func _add_power_strip(position: Vector3, size: Vector3, color: Color, off_energy: float, on_energy: float) -> void:
	var mat := _mat(color, off_energy)
	_box(self, position, size, mat)
	archive_power_materials.append({"material": mat, "off": off_energy, "on": on_energy})


func _process(delta: float) -> void:
	if not _is_archive_powered:
...
func set_archive_powered(powered: bool, immediate := false) -> void:
	_is_archive_powered = powered
	for info in archive_power_lights:
		var light: Light3D = info["node"]
...
func set_bridges_open(open: bool) -> void:
	for bridge_node in bridge_nodes.values():
		bridge_node.visible = open
...
func _transparent_mat(color: Color, emission := 0.0) -> StandardMaterial3D:
	var material := _mat(color, emission)
...
func _torus(parent: Node3D, pos: Vector3, inner_radius: float, outer_radius: float, material: Material) -> MeshInstance3D:
	var instance := MeshInstance3D.new()
...
func _spawn(
	...
	# Scenery goes dark until the Archive is powered, but pieces the player has to
		...
		if mi.mesh:
			for i in mi.mesh.get_surface_count():
				...
				if mat is StandardMaterial3D and (mat as StandardMaterial3D).emission_enabled:
					...
					archive_power_materials.append({"material": dup, "off": 0.05, "on": maxf(orig, 2.5)})
...
func _build_block(parent: Node3D, pos: Vector3, base_mat: Material, accent_mat: Material) -> Node3D:
	...
	if core == null:
		_box(root, Vector3.ZERO, Vector3(0.80, 0.80, 0.80), base_mat)
		_box(root, Vector3(0, 0.415, 0), Vector3(0.56, 0.035, 0.56), accent_mat)
		_box(root, Vector3(0, -0.415, 0), Vector3(0.56, 0.035, 0.56), accent_mat)
		_box(root, Vector3(0, 0, -0.405), Vector3(0.60, 0.035, 0.025), accent_mat)
		_box(root, Vector3(0, 0, 0.405), Vector3(0.60, 0.035, 0.025), accent_mat)
...
func _build_door(parent: Node3D, pos: Vector3, base_mat: Material, accent_mat: Material) -> Node3D:
	...
	root.position = pos
	parent.add_child(root)
	_box(root, Vector3.ZERO, Vector3(0.88, 1.08, 0.22), base_mat)
	_box(root, Vector3(0, 0, -0.125), Vector3(0.58, 0.72, 0.035), accent_mat)
	_box(root, Vector3(0, 0.42, -0.145), Vector3(0.72, 0.055, 0.025), accent_mat)
	return root
...
func _build_player_procedural(parent: Node3D, pos: Vector3, player_mat: Material, accent_mat: Material) -> Node3D:
	...
	root.position = pos
	parent.add_child(root)
	_box(root, Vector3(-0.13, -0.23, 0), Vector3(0.15, 0.28, 0.16), player_mat)
	_box(root, Vector3(0.13, -0.23, 0), Vector3(0.15, 0.28, 0.16), player_mat)
	_box(root, Vector3(0, 0.02, 0), Vector3(0.52, 0.44, 0.34), player_mat)
	_box(root, Vector3(0, 0.05, 0.18), Vector3(0.16, 0.16, 0.02), accent_mat)
	_box(root, Vector3(-0.34, 0.02, 0), Vector3(0.12, 0.34, 0.14), player_mat)
	_box(root, Vector3(0.34, 0.02, 0), Vector3(0.12, 0.34, 0.14), player_mat)
	_box(root, Vector3(0, 0.28, 0), Vector3(0.16, 0.08, 0.16), accent_mat)
	_box(root, Vector3(0, 0.42, 0), Vector3(0.36, 0.30, 0.30), player_mat)
	_box(root, Vector3(-0.08, 0.44, 0.16), Vector3(0.08, 0.08, 0.03), accent_mat)
	_box(root, Vector3(0.08, 0.44, 0.16), Vector3(0.08, 0.08, 0.03), accent_mat)
	_cylinder(root, Vector3(0, 0.66, 0), 0.02, 0.14, accent_mat)
	_sphere(root, Vector3(0, 0.75, 0), 0.05, accent_mat)
	return root
